...
export default function QuizTakePage() {
     ...
     (async () => {
       try {
         ...
         if (!data?.success) {
           ...
           showMessage && showMessage(data?.message || "Failed to load quiz", "error");
           setLoading(false);
           return;
         }
         const q = data.quiz;
         ...
         ps = ps.map((pg, idx) => ({ ...pg, id: String(pg.id ?? `page-${idx + 1}`) }));
 
         setPages(ps);
         // compute total items
         const cnt = ps.reduce((acc, p) => acc + ((p.questions && p.questions.length) || 0), 0);
  ...
  useEffect(() => {
   ...
   const onVisibility = () => { if (!document.hidden) fetchQuiz(); };
  ...
  async function startAttempt() {
    ...
    setStarting(true);
    try {
      const res = await fetch(`${API_BASE}/quizes/${classCode}/quizzes/${quizId}/attempt`, {
        method: "POST",
      ...
      await fetchQuiz();
      const exp = data.expiresAt ? new Date(data.expiresAt) : (quiz.time_limit_seconds ? new Date(Date.now() + quiz.time_limit_seconds * 1000) : null);
      setExpiresAt(exp);
      if (exp) {
  ...
  async function submitAttempt() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/quizes/${classCode}/quizzes/${quizId}/submit`, {
        method: "POST",
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}`, "Content-Type": "application/json" },
  ...
  if (loading) return <section className="quiz-card"><div style={{ padding: 16 }}>Loading quiz…</div>{messageComponent}</section>;
  if (!quiz) return <section className="quiz-card"><div style={{ padding: 16 }}>Quiz not found</div>{messageComponent}</section>;
      ...
      <section className="quiz-card">
        <div className="quiz-header">
          <div>
            ...
            <div className="quiz-meta">
              ...
              Attempts: {quiz.attempts_remaining != null ? `${quiz.attempts_remaining} remaining of ${quiz.attempts_allowed}` : (quiz.attempts_allowed ?? "—")} •
              Time: {quiz.time_limit_seconds ? `${Math.ceil(quiz.time_limit_seconds / 60)} min` : "—"}
            </div>
          </div>

          <div style={{ display: "flex", gap: 8, alignItems: "center", marginLeft: "1rem" }}>
            <button className="quiz-action-btn" onClick={() => setShowDetails((s) => !s)}>
        ...
        {showDetails && (
          <div style={{ padding: 12, border: "1px solid #eee", borderRadius: 6, marginBottom: 12 }}>
            ...
            <div><strong>Items:</strong> {questionsCount}</div>
            <div><strong>Attempts allowed:</strong> {quiz.attempts_allowed ?? "—"}</div>
            <div><strong>Time limit:</strong> {quiz.time_limit_seconds ? `${Math.ceil(quiz.time_limit_seconds / 60)} min` : "—"}</div>
            <div style={{ marginTop: 8, color: "#666" }}>{quiz.description}</div>
          </div>
        )}
          ...
          <div className="right">
            {attemptId ? (
              <div>
                ...
                <div className="timer">Time left: {formatMs(timeLeftMs)}</div>
              </div>
            ) : (
        ...
        {showWarning && (
          <div className="quiz-warning">
            ...
            <p>
              You have a limited number of attempts ({quiz.attempts_allowed ?? "—"}). When you start, your attempt timer will begin. You will see one page at a time. Make sure you are ready before starting.
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              <button className="quiz-button" onClick={() => navigate("/home")}>Cancel</button>
              <button
        ...
        {result && (
          <div className="quiz-result">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button className="quiz-button" onClick={() => navigate("/home")}>Back</button>
            </div>
          </div>
        )}
        ...
        {!showWarning && !result && (
          <div className="quiz-page">
                ...
                <div key={q.id} className="quiz-question">
                  ...
                  {q.type === "short_answer" && (
                    <div>
                      ...
                      />
                      <div className="small-muted">Sentences allowed: {q.sentenceLimit ?? 1}</div>
                    </div>
                  )}
                  ...
                  {q.type === "paragraph" && (
                    <div>
                      ...
                      />
                      <div className="small-muted">Sentences required: {q.sentenceLimit ?? 3}</div>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="quiz-controls">
              <div>
                <button className="quiz-action-btn" onClick={() => setCurrentPage((p) => Math.max(0, p - 1))} disabled={currentPage === 0}>
                  ← Prev
                </button>
              </div>

              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <div className="small-muted">Page {currentPage + 1} / {pages.length}</div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const ok = await confirm({
                        ...
                        message: "Submit attempt now? This will end your attempt.",
                        confirmText: "Submit",
                        cancelText: "Cancel",
                      });
                      if (ok) submitAttempt();
