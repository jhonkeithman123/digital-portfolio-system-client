...
const InvNotificationMenu = ({ invites = [], setInvites, onJoin, onClose, anchorRef }) => {
    ...
    const hideInvite = async (inviteId) => {
        ...
        try {
            const response = await fetch(
                `${reactAppUrl}/classrooms/invites/${inviteId}/hide`, 
                {
                    method: 'POST',
                    headers: {
            ...
            if (response.ok) {
                setInvites(prev => {
                    ...
                    if (next.length === 0 && typeof onClose === 'function') {
                        onClose()
                    }
                });
            }
    ...
    return (
        ...
        >
            <div className="inv-notification-wrapper" onClick={(e) => e.stopPropagation()}>
                {invites.length === 0 ? (
                    <div className="inv-notification">
                        <div className="inv-content">
                            <h3>No invitations</h3>
                            <p className="inv-details">You have no invitations at this time.</p>
                            <div className="inv-actions">
                                <button
                                    className="inv-dismiss"
                                    onClick={() => { setInvites([]); if (onClose) onClose(); }}
                                >
                                    Close
                                </button>
                            </div>
                    ...
                    invites.map(invite => (
                        <div key={invite.id} className="inv-notification">
                            <div className="inv-content">
                                ...
                                <p>You've been invited to join <strong>{invite.classroomName}</strong></p>
